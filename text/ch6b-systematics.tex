...
\item[Cross-section modelling.] Our Monte Carlo simulation relies on the neutrino generator provided by the GENIE collaboration \cite{Andreopoulos:2009rq}. This generator can be configured to use different physics models and interaction parameters \mccorrect{(e.g. axial mass in a CCQE interaction, cross-section normalisation factors, etc.)}, which can affect the relative abundance and the energy of the particles in the final state. The parameters and the physics models are described in detail in the GENIE Physics Manual \cite{Andreopoulos:2015wxa}.
