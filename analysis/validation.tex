...
Figure \ref{fig:numi_nue} shows the number of selected events in data and Monte Carlo for $2.3\times10^{20}$ NuMI POT, collected between February and June 2016 in neutrino mode. Proper systematic uncertainties for NuMI events still need to be assessed, since the off-axis positioning introduces a large uncertainty in the flux that needs to be carefully evaluated. \mccorrect{The overall systematic uncertainty on the number of  BNB selected events before any cut is 28.2\%, as described in detail in Chapter} \ref{sec:systematics}. A conservative estimate gives an expected 30\% systematic uncertainty in the number of selected NuMI events\mccorrect{ after the rectangular cuts}.
